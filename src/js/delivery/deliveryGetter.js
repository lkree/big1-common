...
(function() {
  ...
  // (function() {
  //   const $button = $('.showDeliveryBtn');
  //
  //   function togglerHandler(evt, button) { //togglerHandler
  //     evt.preventDefault();
  //
  //     $('#delivery-toggler').toggleClass('move-toggler');
  //     $('.b-nep-col-tchars div').first().toggleClass('hidden');
  //
  //     button.text() === 'Раскрыть' ? button.text('Свернуть') : button.text('Раскрыть');
  //   }
  //
  //   $button.on('click', (evt) => togglerHandler(evt.originalEvent, $button));
  // })();
  ...
  class DevCompany {
    ...
    renderTD(type, city, deliveryTerms, daysToSend, timeToReserve, notes) {
      ...
      const $tableDescription = $('.delivery-table-wrapper p'),
            $table = $('.deliveryTable');
      ...
      if ($table.hasClass('hidden')) {
        $tableDescription.removeClass('hidden');
  ...
  const pek = new DevCompany('https://cors-anywhere.herokuapp.com/http://www.pecom.ru/ru/calc/towns.php', `https://calc.pecom.ru/bitrix/components/pecom/calc/ajax.php?places[0][]=1&places[0][]=1&places[0][]=1&places[0][]=1&places[0][]=1&places[0][]=1&places[0][]=1&take[town]=-483&deliver[town]=????`, ['ПЭК', , , 'Пн, Вт, Ср, Чт, Пт', 'До 10:00 дня отправки', 'Отправка товаров на доставку ежедневно, кроме субботы и воскресенья, в 12:00']);
  ...
  const kit = new DevCompany('https://capi.gtdel.com/1.0/tdd/city/get-list?token=Xcks5XZW4esL9us7s0fkIKNIF_tDQfQE', 'https://cors-anywhere.herokuapp.com/https://capi.gtdel.com/1.0/order/calculate?token=Xcks5XZW4esL9us7s0fkIKNIF_tDQfQE', ['КИТ', , , 'Пн, Вт, Ср, Чт, Пт', 'До 22:00 дня, предыдущего отправке', 'Отправка товаров на доставку каждую среду в 08:00'], 'POST', '{"city_pickup_code":"760000100000","city_delivery_code":"????","declared_price":"1","places":[{"count_place":"1","height":"1","width":"1","length":"1","weight":"1"}]}', ['content-type', 'application/json']);
  ...
  const dellin = new DevCompany('/scripts/delivery/dellinCities.json', 'https://cors-anywhere.herokuapp.com/https://api.dellin.ru/v1/public/calculator.json', ['Деловые линии', , , 'Пн, Вт, Ср, Чт, Пт', 'До 10:00 дня отправки', 'Отправка товаров на доставку ежедневно, кроме субботы и воскресенья, в 12:00'], 'POST', '{"appkey":"0FB5AE86-A71B-401D-A8CA-A922396510FB","derivalPoint":"7600000100000000000000000","arrivalPoint":"????","sizedVolume":1,"sizedWeight":1}', ['content-type', 'application/json']);
  ...
  if (document.querySelector('.delivery-table-wrapper')) deliveryGetter.inner();
})();