window.deliveryPickup = () => {
  const getDeliveryPoints = () => {
    ...
    return obj && obj.branches ? obj.branches : {offices: []};
  };
  const getOfficesOrPoints = (data) => {
    const arr = data.map(e => e.name.split(', '));
  ...
  const buttonHandler = (options) => {
    ...
    const module = ({pickupModule, pointsWrapper, offices, points, chosenCity}) => {
      module.initiate = () => {
        const handle = (pickupModule, pointsWrapper, offices, points, chosenCity) => {
          ...
          handle.fillModule = () => {
            ...
            if (!isRendered) {
              w()
                .render();
              Array.isArray(offices['city']) ?
                offices['city'].forEach((el, i) => pointsWrapper.append(createPoint(el, offices['address'][i]))) :
                pointsWrapper.append(createNoPoint(offices['city'], offices['address']));
        ...
        handle(pickupModule, pointsWrapper, offices, points, chosenCity)
          .checkRender()
          .fillModule()
          .getUserCity()
          .setUserCity();
